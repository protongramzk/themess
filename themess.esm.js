/* MonetEngine v2 - Refactored Color Extraction Engine
 * 
 * Features:
...
class MonetEngine {
    ...
    static async get(imageUrl, options = {}) {
        try {
            ...
            console.error('MonetEngine extraction failed:', error);
            return this._getFallbackPalette();
        }
    }
 ...
 * 
 * Kompatibel dengan MonetEngine v2 output
 * Input: Palette object dari MonetEngine.get()
 * Output: Shade tones tetap sama seperti v1
 * 
 * Format input dari MonetEngine v2:
 * {
...
class ShadesGenerator {
    ...
    /**
     * Generate shades dari MonetEngine v2 output
     * 
     * @param {Object} palette - Output dari MonetEngine.get()
    ...
    static generate(palette, options = {}) {
        ...
        // Extract OKLch colors dari MonetEngine v2 format
    ...
    /**
     * Extract OKLch color dari format MonetEngine v2
     * Support both new format dan legacy format untuk backward compatibility
     * 
     * @param {Object} color - Color object dari MonetEngine
...
// Contoh output dari MonetEngine.get()
const palette = {
  primary: { oklch: { L: 0.45, C: 1, h: 30 } }, // Oranye
  secondary: { oklch: { L: 0.5, C: 1, h: 170 } }, // Ungu
  accent: { oklch: { L: 0.4, C: 0.2, h: 20 } },
  neutral: { oklch: { L: 0.5, C: 0.01, h: 220 } }
};

// Generate shades
const shades = ShadesGenerator.generate(palette);

// Loop melalui shades dan print ke console log dengan warna ANSI
for (const [name, shade] of Object.entries(shades)) {
  console.log(`\n${name}:`);
  for (const [tone, color] of Object.entries(shade)) {
    const rgb = MonetEngine.oklchToRgb(
      parseFloat(color.match(/oklch\((.*?) (.*?) (.*?)\)/)[1]),
      parseFloat(color.match(/oklch\((.*?) (.*?) (.*?)\)/)[2]),
      parseFloat(color.match(/oklch\((.*?) (.*?) (.*?)\)/)[3])
    );
    const ansiBgColor = `\x1b[48;2;${rgb[0]};${rgb[1]};${rgb[2]}m`;
    const ansiFgColor = `\x1b[38;2;${MonetEngine.oklchToRgb(
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[1] || 1),
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[2] || 0.1),
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[3] || 0)
    )[0]};${MonetEngine.oklchToRgb(
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[1] || 1),
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[2] || 0.1),
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[3] || 0)
    )[1]};${MonetEngine.oklchToRgb(
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[1] || 1),
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[2] || 0.1),
      parseFloat(shade[tone + 10]?.match(/oklch\((.*?) (.*?) (.*?)\)/)[3] || 0)
    )[2]}m`;
    console.log(`${ansiBgColor}${ansiFgColor} ${tone}: ${color} \x1b[0m`);
  }
}
